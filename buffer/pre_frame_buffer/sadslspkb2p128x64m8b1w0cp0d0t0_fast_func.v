...
//  Date of Generation : Fri Mar 04 16:20:12 CST 2022                  
//                                                                     
//---------------------------------------------------------------------
//   --------------------------------------------------------------     
...
//                                                                    
//   Memory Name:sadslspkb2p128x64m8b1w0cp0d0t0                       
...
module sadslspkb2p128x64m8b1w0cp0d0t0 ( QA, QB, ADRA, DA, WEA, MEA, CLKA, TEST1A, RMEA, RMA, ADRB, DB, WEB, MEB, CLKB, TEST1B, RMEB, RMB);
...
generic_behav_sadslspkb2p128x64m8b1w0cp0d0t0 #( PreloadFilename, MES_ALL) u0 ( .QA(QA_tmp), .QB(QB_tmp), .ADRA(ADRA), .DA(DA), .WEA(WEA), .MEA(MEA), .CLKA(CLKA), .ADRB(ADRB), .DB(DB), .WEB(WEB), .MEB(MEB), .CLKB(CLKB) );
...
module generic_behav_sadslspkb2p128x64m8b1w0cp0d0t0 (  QA, QB, ADRA, DA, WEA, MEA, CLKA, ADRB, DB, WEB, MEB, CLKB );

parameter PreloadFilename = "init.file";
parameter MES_ALL = "ON";
parameter words = 128, bits = 64, addrbits = 7;
...
assign ADRA_valid = (( ^ADRA === 1'bx ) ? 2'b01 : ( ( ADRA > 7'b1111111 ) ? 2'b10 : 2'b00 ));
...
begin : blk_WritePortA
  ...
  begin
    if ( MES_ALL == "ON" && $realtime != 0 && mes_all_valid )
    begin
      $display("<<WARNING:address is out of range\n RANGE:0 to 127>> at time=%t; instance=%m (RAMS1H)",$realtime);
...
assign ADRB_valid = (( ^ADRB === 1'bx ) ? 2'b01 : ( ( ADRB > 7'b1111111 ) ? 2'b10 : 2'b00 ));
